...
def load_motion_path(filepath):
    ...
    print("Loading Motion Path from file: ", filepath)
...
class SvMotionPathLoadNode(bpy.types.Node, SverchCustomTreeNode):
    '''Motion Path Loader'''
    bl_idname = 'SvMotionPathLoadNode'
    bl_label = 'Load Motion Path'
    bl_description = 'Load Motion Path from file in vector (3) + quaternion (4) format'
    ...
    def update_path_selection(self, context):
        print("Selected path: ", self.selected_motion_path)
        self.file_path = recent_paths_dict[self.selected_motion_path]
        updateNode(self, context)
    ...
    file_path: StringProperty(
        name="File path of the currently selected motion path",
        description="Path file name",
        default="", update=update_path)

    selected_motion_path: EnumProperty(
        name="Selected Motion Path from loaded motion paths",
    ...
    def draw_buttons(self, context, layout):
        row = layout.row()
        row.prop(self, "selected_motion_path", text="")
        row = layout.row()
        # row.prop(self, "file_path")
        col = row.column()
        load_op = col.operator("node.sv_motion_path_file_importer", text="Load")
    ...
    def process(self):
        ...
        vertex_list, quaternion_list = load_motion_path(self.file_path)
        ...
        for i, (k,v) in enumerate(recent_paths_dict.items()):
            if self.selected_motion_path == k:
                index = i
                break
...
class SvMotionPathFileImporterOp(bpy.types.Operator, ImportHelper):

    bl_idname = "node.sv_motion_path_file_importer"
...
def register():
    bpy.utils.register_class(SvMotionPathLoadNode)
    bpy.utils.register_class(SvMotionPathFileImporterOp)


def unregister():
    bpy.utils.unregister_class(SvMotionPathFileImporterOp)
    bpy.utils.unregister_class(SvMotionPathLoadNode)
