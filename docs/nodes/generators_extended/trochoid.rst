...
Functionality
-------------
The trochoid node creates a variety of 2D line patterns traced by a point attached to a moving circle while rolling (without slipping) on a static circle or on a straight line. Some more familiar patterns are the cardioid (heart shape), the cycloid, the deltoid, the astroid (star shape) and the rosette, but in general most of these patterns are spiral like, petal like, ovoidal like and braided like.

Based on the values of the main parameters (radii & height) and on whether the moving circle is rolling on the inside/outside of the static circle or on a straight line, the patterns generated fit into one of three categories: EPI, HYPO and LINE.
...
The hierarchy of various trochoids is:
...
Additional parameters allow to further tweak the curves (phase, turns, resolution, scale and size).
...
Notes:
[1] : In LINE mode the Radius1 is actually infinite and is simply ignored in the generation of the cycloid patterns
